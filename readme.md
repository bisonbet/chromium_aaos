This repo has a patch to enable Chromium to be run on Android Automotive, along with scripts to help ease the build process. 
# Android Setup
```
cd ~/
...
cd chromium
```
Follow the official [Google Chromium guide](https://chromium.googlesource.com/chromium/src/+/main/docs/android_build_instructions.md)  
During the "Setting up the build", run ``` gn args out/Release_arm64``` and copy the contents of [Release_arm64.gn](/Release_arm64.gn).
To build, you could continue to use the official guide but these instructions going forward will use custom build scripts. It is helpful to still have the Chromium guide pulled up while running through this readme. 
# Building
### Dependencies
...
|          store.jks
```
Android build tools are needed for the signing of the aab. I used Android Studio to create my signing key, but I'm sure there is a terminal command to accomplish creating a .jks.  
...
### Making a release
Run ```./build_release.sh```. This process will take hours to complete. This will:
